...
$(function() {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (registerUser.response) {
						$(modalOutput).fadeIn();
						$(modalOutput + ' .registrador__pin').html('PIN: ' + registerUser.pin + '')
					} else {
						//console.log('Error: ' + registerUser.response);
						$('#error-message').html('');
						$('#error-message').fadeIn();
						$('#error-message').html('<span class="form__error">' + registerUser.message + '</span>');
					}
				} 
			});
		} else {
	...
	$('#search-pin').click(function() {
		...
		if ($('#pin').val() != '') {
			...
			$('#error-message').prepend('<span class="form__error">Necesitas ingresar el PIN a buscar.</span>');
		}
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (searchPin.response) {
						...
						$('#output-photo').html('<img src="http://ec2-13-59-176-172.us-east-2.compute.amazonaws.com' + searchPin.photo + '">');
					} else {
						$('#error-message').html('');
						$('#error-message').fadeIn();
						$('#error-message').html('<span class="form__error">' + searchPin.message + '</span>');
					}
				} 
			});
		} else {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (updateVisitor.response) {
						...
						$('#modal-actualizar .registrador__pin').html('Visitante actualizado correctamente.');
					} else {
						$('#error-message').html('');
						$('#error-message').fadeIn();
						$('#error-message').html('<span class="form__error">' + updateVisitor.message + '</span>');
					}
				} 
			});
		} else {
